...
``class Point(metaclass=RegisteringMeta, category="geometry")`` passes ``category="geometry"`` to ``__new__`` and
``__init__``. That is how :class:`~pyTooling.MetaClasses.ExtendedType` receives ``slots=True``, ``mixin=True`` and
``singleton=True``.

.. rubric:: A method defined on the meta-class is a method of the *class*, not of its instances

``RegisteringMeta.Registry`` is callable as ``Point.Registry()``, not as ``point.Registry()`` - the same relation as
an instance method to an object. This is how a "class property" is built, and why
...
.. hint::
   ...
   :class:`~pyTooling.MetaClasses.SlottedObject` is ``metaclass=ExtendedType, slots=True`` under a name, for the
...
.. seealso::
   ...
   :ref:`TUTORIAL/Decorators`
      |rarr| The class-decorator form, and when it is the better choice.
