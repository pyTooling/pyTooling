...
.. list-table::
   ...
   * - :class:`~pyTooling.CLIAbstraction.Program`
     - argument members, :pycode:`ToArgumentList()`, ``__str__``, the executable resolved on ``PATH`` per platform
   ...
   * - :class:`~pyTooling.CLIAbstraction.Executable`
     - all of that, plus :pycode:`StartProcess()`, :pycode:`Send()`, :pycode:`GetLineReader()`,
       :pycode:`Terminate()`, :pycode:`Wait()` and ``ExitCode``
...
.. hint::
   ...
   is trivially testable - :pycode:`ToArgumentList()` returns a list you can assert on - while a class that starts
