...
.. list-table::
   ...
   * - Approach
     ...
     - Costs
   * - **imperative**
     - the module calls :pycode:`register(handler, "version")` itself
     - the call is separate from the thing it registers, so the two drift; and every module has to remember to make
       it
   * - **meta-class / registry**
     ...
     - nothing to remember, but it only reaches **classes**, it forces the user's type to derive from *your*
       base-class, and the data has nowhere to live except more class variables
   * - **attribute**
     - the decorator records itself on the entity, and the attribute class keeps a registry of where it was applied
     - reaches classes, methods *and* functions, carries typed data of its own, and leaves the user's inheritance
       alone
...
.. attention::
   ...
   **The entity is registered when the module is imported**, because that is when the decorator runs. A handler in a
   plug-in module that nothing imports is invisible to :pycode:`GetFunctions()` - which is a feature when plug-ins are
...
A second example: marking testcases
***********************************

Command line handlers are one use; the shape recurs wherever a framework has to find code by what it *means* rather
than by what it is called. pyTooling's own :ref:`test markers <TESTING/Markers>` are the same idea:
...
That is the whole argument for attributes in one example, and it is why the marker in
:mod:`pyTooling.Testing` exists.
...
.. attention::
   ...
   ``versionHandler`` above - is yielded twice by :pycode:`GetFunctions()`. Wrap the result in a :class:`set` (or
...
.. important::
   ...
   **Base and derived registries are separate, not nested.** :pycode:`Command.GetFunctions()` does *not* return the
   ...
   Where sub-class matching *is* wanted, it comes from
   :meth:`~pyTooling.MetaClasses.ExtendedType.GetMethodsWithAttributes` with :pycode:`predicate=Command`, and from
...
.. caution::
   ...
   Note also that ``_scope`` reads back from an attribute **instance** - :pycode:`TestCase("x").Scope` - and not
   from the class.
